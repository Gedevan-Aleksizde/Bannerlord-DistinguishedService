...
namespace DistinguishedServiceRedux.settings
{
    public class Settings : AttributeGlobalSettings<Settings>
    {
        ...
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        ...
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        [SettingPropertyInteger("{=IraZX6JYH}Max. nominations", 1, 128, "0", Order = 2, RequireRestart = false, HintText = "{=xkVDlk1N1V}The maximum number of nominees you are allowed to pick at the end of a battle. Default is 1.")]
        public int MaxNominations { get; set; } = 1; // max_nominations
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        [SettingPropertyInteger("{=Rw8hDdgtq}Min. Tier", -1, 10, "0", Order = 3, RequireRestart = false, HintText = "{=TDkNix5fda}The minimum tier of unit eligible to become a hero. Set to -1 to only allow units with no further upgrades to be nominated. Default is -1.")]
        public int EligibleTier { get; set; } = 4; // tier_threshold
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        [SettingPropertyInteger("{=NqAfmE3I7}Min. Kills for Infantly Troops", 1, 128, "0", Order = 4, RequireRestart = false, HintText = "{=BLpQfo0f8b}The number of kills threshold to be nominated for infantly. Default is 5.")]
        public int EligibleKillCountInfantry { get; set; } = 5; // inf_kill_threshold
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        [SettingPropertyInteger("{=0owPtnEUD}Min. Kills for Mounted Troops", 1, 128, "0", Order = 5, RequireRestart = false, HintText = "{=CRRwgyzJvq}The number of kills threshold to be nominated for mounted troops. Default is 5.")]
        public int EligibleKillCountCavalry { get; set; } = 5; // cav_kill_threshold
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        [SettingPropertyInteger("{=NNQYYgCN3}Min. Kills for Ranged Troops", 1, 128, "0", Order = 6, RequireRestart = false, HintText = "{=vcXgO4B8mG}The number of kills threshold to be nominated for ranged troops. Default is 5.")]
        public int EligibleKillCountRanged { get; set; } = 5; // ran_kill_threshold
        [SettingPropertyGroup("{=KAgbRWUNn}Eligibility")]
        [SettingPropertyFloatingInteger("{=WyI8Nf9yg}Percentile Outperform", 0, 1, "0.000", Order = 7, HintText = "{=aWfPIRIssB}The percentile of kills a unit must exceed to qualify to be nominated. Set to 0 for previous versions' behaviour (only kill thresholds). Default is 0.68.")]
        public float EligiblePercentile { get; set; } = 0.68f;  // outperform_percentile
        [SettingPropertyGroup("{=xwNNXlGcq}Companion Capacity")]
        [SettingPropertyBool("{=7C44pRJyT}Ignore Companion Limit", HintText = "{=XoyKSIk9Yo}If enabled, the number of nominations will excess the native companion limit. NOTE: Your companion will randomly LOST if you have more companions than the limit. Default is Disabled.")]
        ...
        [SettingPropertyInteger("{=JzTC6uoHk}Skill points", 0, 500, Order = 0, HintText = "{=85WomjlJtB}The number of primary skill point bonus to manually assign to newly-created companion skills. Default is 150.")]
        public int AdditionalSkillPoints { get; set; } = 150; // base_additional_skill_points
        [SettingPropertyGroup("{=Lwt72jlJJ}Skills")]
        [SettingPropertyInteger("{=FqySMd4Cu}Skill bonus", 0, 10, Order = 1, HintText = "{=KFwunOsf4k}The number of skill bonuses for players to choose for newly-created heroes. Default is 2.")]
        public int NumSkillBonuses { get; set; } = 2; // number_of_skill_bonuses
        [SettingPropertyGroup("{=Lwt72jlJJ}Skills")]
        [SettingPropertyInteger("{=JEPdqkqB7}Skill rounds", 0, 10, Order = 2, HintText = "{=OfDcCqMwbP}The number of round you can assign skill bonuses during each round gives [base_additional_skill_points/round#] per skill. Default is 1.")]
        public int NumSkillRounds { get; set; } = 1; // number_of_skill_rounds 
        [SettingPropertyGroup("{=Lwt72jlJJ}Skills")]
        [SettingPropertyBool("{=ebrfIjhX9}Randomized skills", IsToggle = true, Order = 3, HintText = "{=17RpeC0Bd7}If enabled, bonus skill is assigned randomly.")]
        ...
        [SettingPropertyGroup("{=Lwt72jlJJ}Skills")]
        [SettingPropertyBool("{=RfH84SuAS}Fill In Perks", IsToggle = true, Order = 6, HintText = "{=UiL3DpJrV1}If enabled, the newly-generated hero's perks are fill in automatically. Default is disabled.")]
        ...
        */
        ...
        [SettingPropertyGroup("{=ASqPAFgkE}Misc")]
        [SettingPropertyBool("{=v71F1OBfOf}Show Warnings", HintText = "{=lVB5RJoCDm}If disabled, the system warning messages are hidden. Default is enabled.")] // UI改善のため値を反転した
